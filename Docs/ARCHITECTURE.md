...
14. **Configuration, Authoring & Developer UX Architecture (Phase 8 & 9 Updates)**:
    ...
    - **Domain-Specific Preset Ownership & Runtime Consumption**: Presets are housed strictly within their owning module domains (`UVoxelStreamingPreset` in `VoxelAssets`, `UVoxelPhysicsPreset` in `VoxelPhysics`). At runtime, `UVoxelWorldSubsystem::ApplyWorldDefinition` consumes `UVoxelPhysicsPreset` (updating `ActiveCollisionMode`, `bActiveAsyncCooking`, and `ActiveCollisionProfileName` across all active collision components and worker jobs) and broadcasts `OnWorldDefinitionApplied` to propagate streaming distance bands and frame budgets cleanly to `UVoxelStreamingManager` without circular module dependencies.
...
15. **Unreal Python Bootstrap & Authoring Automation (Phase 9)**:
    - **Single-Script Project Bootstrap (`SetupVoxelFramework.py`)**: A developer can enable the plugin, run one Unreal Python script via `import VoxelFramework.SetupVoxelFramework as setup; setup.run()` (or `setup.run(overwrite=True)`), and receive a complete, fully wired suite of project-owned `.uasset` configuration files under `/Game/VoxelFramework/`.
    ...
    - **Strict Non-Destructive Idempotency Model**: The script never deletes assets. If assets already exist, `setup.run()` inspects their properties and repairs any uninitialized fields or missing references in-place (`UPDATED`) or skips them if valid (`SKIPPED`). `setup.run(overwrite=True)` updates all configuration properties in-place without breaking UObject identities or external references.
